...
function FindMe() {
    return (
        <div className="max-w-[90%] sm:w-[943px] flex items-center justify-center flex-col mx-auto">
            ...
            <div className=" w-full grid grid-cols-2 sm:grid-cols-3  gap-4 p-4">
                {socials.map((item, index) => (
                    <a
                        key={index}
                        href={item.link}
                        className="group bg-gray-100 flex flex-col items-center justify-center p-4 border rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300"
                        target="_blank"
